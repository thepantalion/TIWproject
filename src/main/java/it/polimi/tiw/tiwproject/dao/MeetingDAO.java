...
public class MeetingDAO {
    ...
    public ArrayList<Meeting> meetingsCreated(User creator) throws SQLException {
        String query = "SELECT meeting.title, meeting.date, meeting.time, meeting.duration FROM db_tiw_project.user JOIN db_tiw_project.meeting " +
                       "ON user.idUser = meeting.idCreator " +
                       "WHERE curdate() <= meeting.date AND curtime() <= meeting.time AND user.idUser = ?";
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
            ...
            try (ResultSet result = preparedStatement.executeQuery()){
                if(!result.isBeforeFirst()) return meetingList;
                else {
                    while(result.next()){
    ...
    public ArrayList<Meeting> meetingsInvited(User invitee) throws SQLException {
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(query)) {
            preparedStatement.setString(1, invitee.getUsername());
